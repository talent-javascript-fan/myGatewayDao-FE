...
function replaceEnvWithRef(policy) {
    const resource = policy.Resource.map(resource => resource.includes('${env}') ? cloudform_types_1.Fn.Sub(resource, { env: cloudform_types_1.Fn.Ref('env') }) : resource);
...
function validateCustomPolicies(data, categoryName, resourceName) {
    ...
    Learn more about custom IAM policies for ${categoryName}: https://docs.amplify.aws/cli/function/#access-existing-aws-resource-from-lambda-function\n`;
        if (validatePolicy && validatePolicy.errors) {
            errorMessage += (_a = validatePolicy.errors) === null || _a === void 0 ? void 0 : _a.map((error) => error.message).join(os_1.EOL);
        }
