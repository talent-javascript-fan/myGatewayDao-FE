...
async function addCFNResourceDependency(context, customResourceName) {
    const selectResourcesInCategory = (choices, currentResourceDependencyMap, category) => ({
        type: 'checkbox',
        name: 'resources',
        message: `${lodash_1.default.capitalize(category)} has ${choices.length} resources in this project. Select the one you would like your custom resource to access`,
        choices,
        default: currentResourceDependencyMap[category],
    });
    const selectCategories = (choices, currentCategoryDependencyMap) => ({
        type: 'checkbox',
        name: 'categories',
        message: 'Select the categories you want this custom resource to have access to.',
        choices,
        validate: answers => (lodash_1.default.isEmpty(answers) ? 'You must select at least one category' : true),
    ...
    const categoryAnswer = await inquirer_1.default.prompt(categoryQuestion);
    const selectedCategories = categoryAnswer.categories;
    const resources = [];
...
function showUsageInformation(resources) {
    amplify_prompts_1.printer.success(`Successfully added dependent resources.`);
    amplify_prompts_1.printer.info(`You can access the dependent resource attributes using the Ref's mentioned below`);
    amplify_prompts_1.printer.blankLine();
    for (const resource of resources) {
        amplify_prompts_1.printer.info(`Category: ${resource.category}  Resource: ${resource.resourceName}`);
        amplify_prompts_1.printer.blankLine();
        for (const attribute of resource.attributes || []) {
            amplify_prompts_1.printer.info(`Attribute ${attribute}`);
            amplify_prompts_1.printer.info(`{ Ref: ${resource.category}${resource.resourceName}${attribute} }`);
            amplify_prompts_1.printer.blankLine();
        }
        amplify_prompts_1.printer.blankLine();
    }
